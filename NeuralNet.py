...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        temp = self.getWeightedSum(inActs)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(length):
            ...
            self.weights[i] += (alpha * (store[i] * delta))

            totalModification += abs(alpha * (store[i] * delta))
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for x in self.layers:
            ...
            for y in x:
                temp2 = y.sigmoidActivation(store2)
                temp.append(temp2)
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    temp = 0.0
                    length = len(nextLayer)
                    for x in range(length):
                        store = nextLayer[x].weights[neuronNum + 1]
                        delta = store * deltas[0][x]
                        temp = delta + temp
                    delta = gPrime * temp
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    temp = enumerate(examplesTest)

    for x, y in temp:

        shape = [round(h) for h in nnet.feedForward(y[0])[-1]]
